...
const PostModal = React.lazy(() =>
  import("./components/modals/PostModal").then((m) => ({
    default: m.PostModal,
  })),
);
...
const CalendarSharing = React.lazy(() =>
  import("./components/CalendarSharing").then((m) => ({
    default: m.CalendarSharing,
  })),
);
const DirectSearch = React.lazy(() =>
  import("./components/DirectSearch").then((m) => ({
    default: m.DirectSearch,
  })),
);
const ExportModal = React.lazy(() =>
  import("./components/modals/ExportModal").then((m) => ({
    default: m.ExportModal,
  })),
);
const ExcelImportModal = React.lazy(() =>
  import("./components/modals/ExcelImportModal").then((m) => ({
    default: m.ExcelImportModal,
  })),
);
const BusinessModal = React.lazy(() =>
  import("./components/modals/BusinessModal").then((m) => ({
    default: m.BusinessModal,
  })),
);
const AutoFillModal = React.lazy(() =>
  import("./components/modals/AutoFillModal").then((m) => ({
    default: m.AutoFillModal,
  })),
);
const WidgetsTab = React.lazy(() =>
  import("./components/CreativeStudioTab").then((m) => ({
    default: m.WidgetsTab,
  })),
);
const AnalyticsTab = React.lazy(() =>
  import("./components/AnalyticsTab").then((m) => ({
...
const BrandKitTab = React.lazy(() =>
  import("./components/BrandKitTab").then((m) => ({ default: m.BrandKitTab })),
);
const IdeasTab = React.lazy(() =>
  import("./components/IdeasTab").then((m) => ({ default: m.IdeasTab })),
);
const WorkspaceManagementTab = React.lazy(() =>
  import("./components/WorkspaceManagementTab").then((m) => ({
    default: m.WorkspaceManagementTab,
  })),
);
function LazyModal({
  isOpen,
...
export default function App() {
      ...
      const resolveShortCode = async () => {
        try {
          ...
          if (!snapshot.empty) {
            ...
            console.log(
              "[App] Short link found. Original URL:",
              data.originalUrl,
            );
            ...
            if (data.originalUrl) {
              ...
              try {
                ...
                if (url.origin === window.location.origin) {
                  ...
                  console.log(
                    "[App] Redirecting to internal path:",
                    internalPath,
                  );
                  navigate(internalPath, { replace: true });
                } else {
                  console.log(
                    "[App] Redirecting to external URL:",
                    data.originalUrl,
                  );
  ...
  const aiSettings =
    useAppStore((state) => state.aiSettings) || initialAiSettings;
  ...
  const analyticsSettings =
    useAppStore((state) => state.analyticsSettings) || initialAnalyticsSettings;
  const setAnalyticsSettingsState = useAppStore(
    (state) => state.setAnalyticsSettings,
  );
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (
        ...
        toast.error(
          "Popup blocked/closed. Please open the app in a new tab (button top right) to sign in securely.",
        );
  ...
  const isViewer = useMemo(
    () =>
      !!(
        user &&
        activeBusiness &&
        activeBusiness.memberRoles?.[user.uid] === "viewer"
      ),
    [user, activeBusiness],
  );
  const isGuest = !user;
  ...
  const [userOnboardingComplete, setUserOnboardingComplete] = useState<
    boolean | null
  >(null);

  const industryConfig = useMemo(
    () => getIndustryConfig(activeBusiness?.industry),
  ...
  const handleAiSettingChange = (key: string, value: string | boolean) => {
    ...
    if (user) {
      ...
      setDoc(userRef, { aiSettings: newSettings }, { merge: true }).catch(
        (err) => {
  ...
  const [settingsTab, setSettingsTab] = useState<
    "account" | "workspaces" | "ai" | "analytics" | "maintenance"
  >("account");
  ...
  useEffect(() => {
    const handleExtensionMessage = async (event: MessageEvent) => {
      ...
      if (event.data?.type === "FORGE_GET_USER_STATE") {
        if (!user) {
          console.warn(
            "[Forge Companion] Extension requested state but user is not logged in.",
          );
          return;
        }
        console.log(
          "[Forge Companion] Extension requested state. Sending:",
          user.email,
        );
        window.postMessage(
          {
      ...
      if (
        (event.data?.type === "FORGE_ADD_NOTE" ||
          event.data?.type === "FORGE_QUICK_NOTE") &&
        user
      ) {
        try {
          // Fallback to activeBusiness if workspaceId wasn't passed by the extension
          const targetWorkspaceId =
            event.data.workspaceId || activeBusiness?.id;

          if (!targetWorkspaceId)
            throw new Error("No active workspace to route into.");
  ...
  const isWidgetsTabActive =
    activeTab === "widgets" || activeTab === "creative";
  ...
  const addSyncLog = (
    ...
    setSyncLogs((prev) =>
      [{ id: uuidv4(), time: new Date(), message, type }, ...prev].slice(
        0,
        100,
      ),
    );
  ...
  const [builtInStatus, setBuiltInStatus] = useState<BuiltInAiStatus>({
    isLoaded: false,
    isLoading: false,
    visionIsLoaded: false,
    visionIsLoading: false,
    isProcessing: false,
    progress: 0,
    message: "",
    error: null,
  ...
  const [initialProductsForModal, setInitialProductsForModal] = useState<
    HighStockProduct[]
  >([]);
  ...
  const [pendingShareData, setPendingShareData] = useState<Business | null>(
    null,
  );
  ...
  const completeShareAccess = async (bizData: Business) => {
    ...
    const snapshot = await getDocs(q);
    const sharedPosts = snapshot.docs.map(
    ...
    if (bizData.shareFilters) {
      if (bizData.shareFilters.tags?.length) {
        filteredPosts = filteredPosts.filter((p) =>
          bizData.shareFilters?.tags?.includes(p.outlet || ""),
        );
      }
      if (bizData.shareFilters.dateRange?.start) {
        filteredPosts = filteredPosts.filter(
          (p) => p.date >= bizData.shareFilters!.dateRange!.start,
        );
      }
      if (bizData.shareFilters.dateRange?.end) {
        filteredPosts = filteredPosts.filter(
          (p) => p.date <= bizData.shareFilters!.dateRange!.end,
        );
      }
    }
        ...
        try {
          ...
          if (bizDoc.exists()) {
            ...
            if (bizData.shareToken === shareToken) {
              if (
                bizData.shareExpiresAt &&
                isAfter(new Date(), parseISO(bizData.shareExpiresAt))
              ) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const bizList = snapshot.docs.map(
        ...
        if (bizList.length > 0) {
          ...
          if (
            !activeBusiness ||
            !bizList.find((b) => b.id === activeBusiness.id)
          ) {
  ...
  const handleStartFinetune = async () => {
    try {
      await axios.post("/api/ai/finetune", {
        modelId: aiSettings.builtinModelId || "LLaMA-7B",
      });
      ...
      toast.error(
        error.response?.data?.error || "Failed to start fine-tuning.",
      );
    }
  };
  ...
  const handleAutoCategorizeAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < uncategorized.length; i += batchSize) {
        ...
        const categoriesMap =
          await generateAppJson(`Categorize the following products into one of these categories: Furniture, Building Materials, Home Appliances, Kitchenware, Electronics, Lighting, Bathroom Fittings, Hardware.
      ...
      toast.success(
        `Successfully categorized ${uncategorized.length} products!`,
      );
    } catch (error) {
  ...
  const [dropActionPrompt, setDropActionPrompt] = useState<{
    dateStr: string;
    files: File[];
  } | null>(null);
    ...
    try {
      ...
      if (post.images && post.images.length > 0) {
        for (let i = 0; i < post.images.length; i++) {
          ...
          if (!img) {
            console.warn(
              `[handleSavePost] Skipping null/undefined image at index ${i}`,
            );
            continue;
          }
          ...
          if (trimmed.startsWith("data:")) {
            ...
            console.warn(
              `[handleSavePost] Skipping unrecognized image format at index ${i}:`,
          ...
          try {
            ...
            const timeoutPromise = new Promise<never>((_, reject) =>
              setTimeout(
                () => reject(new Error("Upload timeout after 120s")),
                120000,
              ),
            );
            ...
            console.error(
              `[handleSavePost] Failed to upload image at index ${i}:`,
              uploadErr,
            );
            addSyncLog(
              `Warning: Image ${i + 1} could not be uploaded and was skipped`,
              "error",
            );
      ...
      const postWithUser = {
        ...
        businessId:
          post.businessId ||
          (calendarMode === "personal" ? "personal" : activeBusiness?.id || ""),
        images: imageUrls,
      };
      ...
      addSyncLog(
        `Successfully saved post: ${post.title || "Untitled"}`,
        "success",
      );
      ...
      if (activeBusiness?.id) {
        try {
          ...
          const existingNames = new Set(
            currentCats.map((c) => `${c.type}:${c.name}`),
          );
          const newCats: any[] = [];

          if (
            post.productCategory &&
            !existingNames.has(`category:${post.productCategory}`)
          ) {
            newCats.push({
              id: uuidv4(),
              name: post.productCategory,
              type: "category",
              enabled: true,
            });
          }
          ...
          if (newCats.length > 0) {
            await setDoc(
              catRef,
              { categories: [...currentCats, ...newCats] },
              { merge: true },
            );
            addSyncLog(
              `Auto-synced ${newCats.length} new categories to Brand Kit`,
              "info",
            );
          }
        } catch (e) {
  ...
  const handlePublishPost = async (post: Post) => {
    ...
    if (!imageUrl) {
      alert(
        "This post has no uploaded image. Please add an image before publishing.",
      );
      return;
    }
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((p) => (p.id === post.id ? { ...p, ...update } : p)),
      );
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const checkScheduledPosts = async () => {
      ...
      try {
        ...
        const repeatPosts = posts.filter(
          (p) =>
            p.repeatEnabled &&
        ...
        if (repeatPosts.length > 0) {
          ...
          for (const post of repeatPosts) {
            ...
            const intervalDays =
              post.repeatInterval === "weekly"
                ? 7
                : post.repeatInterval === "biweekly"
                  ? 14
                  : 30;
            ...
            if (nextDate <= now) {
              ...
              batch.set(doc(db, "posts", newPostId), {
                ...newPost,
                userId: user.uid,
              });
              ...
              if (batchCount >= 40) {
                // Firestore batch limit is 500, but we stay safe
                await batch.commit();
                batch = writeBatch(db);
                batchCount = 0;
              }
  ...
  const handleBulkImport = async (csvText: string) => {
    ...
    if (imported.length === 0) {
      toast.error(
        "No valid posts found in CSV. Required columns: date, caption. Optional: outlet, type, title, brief, hashtags, image, scheduledTime",
      );
      return;
    }
  ...
  const handleDeletePost = async (id: string) => {
    ...
    try {
      ...
      addSyncLog(
        `Successfully deleted post: ${postToDelete?.title || id}`,
        "success",
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const currentDeps = {
      ...
      sharedBusiness: sharedBusiness?.id,
      isViewOnly,
      calendarMode,
    };
    const changedDeps = Object.keys(currentDeps).filter(
      (k) => (currentDeps as any)[k] !== prevDeps.current[k],
    );
    ...
    const context =
      calendarMode === "personal"
        ...
        : activeBusiness
          ...
          : isViewOnly && sharedBusiness
            ? `shared business ${sharedBusiness.id}`
            : "no context";
    ...
    if (isViewOnly && sharedBusiness && sharedBusiness.id) {
      ...
      q = query(
        collection(db, "posts"),
        where("businessId", "==", sharedBusiness.id),
      );
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        console.log(
          `[Sync] Snapshot received for ${context}, metadata.fromCache: ${snapshot.metadata.fromCache}`,
        );
        const fetchedPosts = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as Post,
        );
        addSyncLog(
          `Received update from cloud: ${fetchedPosts.length} posts`,
          "info",
        );
        console.log(
          `[Sync] Received ${fetchedPosts.length} posts from Firestore`,
        );
  ...
  const handleConnectGoogleDrive = async () => {
    try {
      ...
      const response = await fetch(
        `/api/auth/google/url?${queryParams.toString()}`,
      );
      ...
      if (
        ...
        if (lastNotified !== todayStr) {
          try {
            ...
            console.warn(
              "Notifications constructor not supported in this browser",
              e,
            );
          }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (
      overId === "floating-chat-button" ||
      overId === "floating-chat-container"
    ) {
    ...
    if (activeData?.type === "product") {
      ...
      const newPost: Post = {
        id: uuidv4(),
        date: targetDate,
        outlet: product.type.includes("Living")
          ? "Forge Living Mall"
          : product.type.includes("Office")
            ? "Forge Office System"
            : "Forge Buildware",
        type: `🔴 ${product.type}`,
        title: product.title,
    ...
    if (activeData?.type === "notebook-block") {
      ...
      const newPost: Post = {
        ...
        hashtags: activeBusiness?.industry
          ? `#${activeBusiness.industry.replace(/\s+/g, "")}`
          : "",
      ...
      handleSavePost(newPost);
      toast.success(
        `Created post from ${block.type === "postcard" ? "postcard" : "notebook block"}`,
      );
      return;
    }
  ...
  const processDroppedFiles = async (
    ...
    if (mode === "single") {
      ...
      try {
        ...
        const match = collageBase64.match(
        ...
        const errMsg =
          error instanceof Error
            ? error.message
            : "Failed to auto-generate content.";
        toast.error(errMsg);
        await handleSavePost({
          ...placeholderPost,
  ...
  const handleAutoFillSubmit = async (prompt: string, count: number) => {
    ...
    const loadingToast = toast.loading(
      `AI is brainstorming ${count} posts for the month... This might take a minute or two.`,
    );

    try {
      // First generate the ideas
      addSyncLog(
        `Requesting ${count} posts from AI for campaign: ${prompt}`,
        "info",
      );
      ...
      if (generatedPosts.length > 0) {
        ...
        toast.success(
          `Successfully auto-filled ${Math.min(generatedPosts.length, count)} posts!`,
        );
  ...
  const handleGenerateMockup = async (post: Post) => {
    if (!post.title || !post.brief || !post.caption) {
      toast.warning(
        "Post needs a title, brief, and caption to generate a mockup.",
      );
      return;
    }
    try {
  ...
  const exportToExcel = async (settings: ExportSettings) => {
    const { startMonth, endMonth, visibleFields, layoutStyle, accentColor } =
      settings;
    ...
    try {
      toast.loading("Analyzing workspace data for export...", {
        id: "excel-export",
      });
      ...
      const requestedMonthLabelsOriginal = intervalMonths.map((m) =>
        format(m, "MMM yyyy"),
      );
      const requestedMonthLabelsLower = requestedMonthLabelsOriginal.map((l) =>
        l.toLowerCase().trim(),
      );
      ...
      if (workbook.worksheets.length === 0) {
        throw new Error(
          `The requested months (${requestedMonthLabelsOriginal.join(", ")}) are not available.`,
        );
      }
      ...
      for (const monthDate of intervalMonths) {
        ...
        let sheet = workbook.worksheets.find(
          (s) => s.name.toLowerCase().trim() === searchLabel,
        );
        if (!sheet) continue;
        ...
        monthPosts.forEach((post) => {
          ...
          sheet.getCell(rBase, col).value =
            `  ${format(dateObj, "EEE").toUpperCase()}  ${format(dateObj, "dd")}`;
              ...
              row.eachCell((cell) => {
                cell.style = {
                  ...
                  fill: {
                    type: "pattern",
                    pattern: "solid",
                    fgColor: { argb: themeColors.darkBg },
                  },
                  font: {
                    ...cell.font,
                    color: { argb: themeColors.darkText },
                    italic: false,
                  },
                };
              });
      ...
      saveAs(
        new Blob([buffer]),
        `Forge_Export_${activeBusiness?.name?.replace(/\s+/g, "_") || "Calendar"}.xlsx`,
      );
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (jsonData.length === 0) {
          toast.error(
            "No valid data found in Excel file. If you exported a Calendar view, please ensure the file contains the hidden _DATA_ sheet.",
          );
          return;
        }
        ...
        if (preferredProvider === "groq" || preferredProvider === "gemini") {
          ...
          if (mapping && Object.keys(mapping).length > 0) {
            ...
            addSyncLog(
              "AI mapping failed or returned no data, falling back to manual mapping...",
              "info",
            );
        ...
        function manualMapRow(row: any) {
          const findVal = (keys: string[]) => {
            const key = Object.keys(row).find((k) =>
              keys.includes(k.toUpperCase().replace(/\s/g, "_")),
            );
            return key ? row[key] : "";
          };
          ...
          return {
            ...
            date:
              findVal(["DATE", "POST_DATE"]) ||
              format(new Date(), "yyyy-MM-dd"),
            outlet:
              findVal(["OUTLET", "PLATFORM", "POST_OUTLET"]) || "Buildware",
            productCategory: findVal([
              "CATEGORY",
              "PRODUCT_CATEGORY",
              "PRODUCT_TYPE",
            ]),
            ...
            title:
              findVal(["TITLE", "HEADLINE", "POST_TITLE"]) || "Untitled Post",
        ...
        if (user) {
          setIsSyncing(true);
          addSyncLog(
            `Starting import of ${importedPosts.length} posts from Excel...`,
            "info",
          );
          ...
          try {
            ...
            for (let index = 0; index < importedPosts.length; index++) {
              ...
              addSyncLog(
                `Processing Excel post ${index + 1}/${importedPosts.length}: ${post.title}`,
                "info",
              );
              ...
              if (post.images && post.images.length > 0) {
                for (let i = 0; i < post.images.length; i++) {
                  ...
                  if (trimmed.startsWith("data:") || trimmed.length > 10000) {
                    let dataUrl = trimmed.startsWith("data:")
                      ? trimmed
                    ...
                    try {
                      addSyncLog(
                        `  Uploading image ${i + 1} from Excel...`,
                        "info",
                      );
                      ...
                      addSyncLog(
                        `  Failed to upload image ${i + 1} for post ${postId}`,
                        "error",
                      );
                    }
                  } else {
                    // Unrecognized, skip
                    console.warn(
                      `[importScheduleExcel] Skipping unrecognized image format:`,
                      trimmed.substring(0, 50),
                    );
                  }
                }
              ...
              processedPosts.push({
                ...post,
                userId: user.uid,
                businessId: activeBusiness?.id || post.businessId,
                images: finalImageUrls,
              });
            }

            addSyncLog(
              `Saving ${processedPosts.length} posts to cloud...`,
              "info",
            );
            ...
            addSyncLog(
              `Successfully imported ${processedPosts.length} posts from Excel`,
              "success",
            );
        ...
        toast.error(
          "Failed to parse Excel file. Make sure it is a valid .xlsx or .xls file.",
        );
      } finally {
        setIsSyncing(false);
      }
      ...
      try {
        ...
        if (data.posts && Array.isArray(data.posts)) {
          ...
          if (user) {
            setIsSyncing(true);
            addSyncLog(
              `Starting import of ${data.posts.length} posts...`,
              "info",
            );
            try {
              ...
              for (let index = 0; index < data.posts.length; index++) {
                ...
                addSyncLog(
                  `Processing post ${index + 1}/${data.posts.length}: ${post.title || "Untitled"}`,
                  "info",
                );
                ...
                if (post.images && post.images.length > 0) {
                  for (let i = 0; i < post.images.length; i++) {
                    ...
                    if (!img) {
                      console.warn(
                        `[importScheduleJson] Skipping null/undefined image at index ${i}`,
                      );
                      continue;
                    }
                    ...
                    if (trimmed.startsWith("data:")) {
                      ...
                      console.warn(
                        `[importScheduleJson] Skipping unrecognized image format at index ${i}:`,
                        trimmed.substring(0, 50),
                      );
                      continue;
                    }
                    ...
                    try {
                      ...
                      addSyncLog(
                        `  Uploading image ${i + 1}/${post.images.length} (${sizeInMB.toFixed(2)} MB)...`,
                        "info",
                      );
                      ...
                      while (retries >= 0) {
                        try {
                          ...
                          retries--;
                          addSyncLog(
                            `  Retrying upload for image ${i + 1}... (${2 - retries}/2)`,
                            "info",
                          );
                      ...
                      addSyncLog(
                        `  Failed to upload image ${i + 1} for post ${postId}: ${errorMsg}`,
                        "error",
                      );
              ...
              addSyncLog(
                `All images processed. Saving ${processedPosts.length} posts to cloud...`,
                "info",
              );
              ...
              for (let i = 0; i < processedPosts.length; i += BATCH_SIZE) {
                ...
                await batch.commit();
                addSyncLog(
                  `Saved batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(processedPosts.length / BATCH_SIZE)}`,
                  "info",
                );
              }
              addSyncLog(
                `Successfully imported and synced ${processedPosts.length} posts`,
                "success",
              );
      ...
      <div className="min-h-screen bg-[#1A1C1E] flex items-center justify-center p-4">
        <div className="bg-[#24262B] border border-[#33353B] p-8 rounded-[24px] text-center max-w-md ">
          ...
          <p className="text-[#909094] mb-6">
            We're having trouble connecting to the cloud. This might be due to a
            slow network or a temporary service issue.
          </p>
  ...
  const exportProductExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(
        wb,
        `Forge_Stock_Check_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
    } catch (e) {
  ...
  const handleExportExtensionZip = async () => {
    try {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (
              ...
              console.warn(
                `Failed to fetch true ${file} (received SPA fallback).`,
              );
              continue;
            }
        ...
        JSON.stringify(
          {
            ...
            version: "1.0.0",
            description:
              activeBusiness?.description || "Exported from Forge AI",
      ...
      zip.file(
        ...
        JSON.stringify(
          localStorage.getItem(
            `rainbowStockCheck_${activeBusiness?.id || "default"}`,
          )
            ? JSON.parse(
                localStorage.getItem(
                  `rainbowStockCheck_${activeBusiness?.id || "default"}`,
      ...
      saveAs(
        content,
        `Forge_Companion_Extension_${format(new Date(), "yyyy-MM-dd")}.zip`,
      );
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.categoryCounts && Array.isArray(data.categoryCounts)) {
          localStorage.setItem(
            `rainbowCategoryCounts_${activeBusiness?.id || "default"}`,
            JSON.stringify(data.categoryCounts),
          );
        ...
        toast.success(
          "Product data imported successfully! Please refresh or switch tabs to see changes.",
        );
    ...
    try {
      ...
      await setDoc(doc(db, "brand_kits", bizId), {
        ...
        colors: [
          {
            name: "Primary",
            hex: (data as any).brandColors?.primary || "#3b82f6",
          },
          {
            name: "Secondary",
            hex: (data as any).brandColors?.secondary || "#1e293b",
          },
          {
            name: "Accent",
            hex: (data as any).brandColors?.accent || "#f59e0b",
          },
        ],
        fonts: {
          heading: "Inter",
      ...
      ensureLocalAiEnginesReady()
        ...
        .catch(() =>
          toast.success(
            "Workspace created — open Settings to finish loading local AI.",
          ),
        );
        ...
        <ConfigWorkspaceProvider activeBusiness={activeBusiness}>
          ...
          <DndContext
            ...
            onDragOver={(event) => {
              ...
              if (
                over &&
                (over.id === "calendar-tab-droppable" ||
                  over.id === "calendar-tab-droppable-mobile")
              ) {
            ...
            <div className="min-h-[100dvh] w-full bg-white dark:bg-[#191919] flex flex-col font-sans text-[#37352F] dark:text-[#EBE9ED] relative print:h-auto print:overflow-visible print:bg-white">
              ...
              >
                <div className="flex flex-1 w-full relative">
                      ...
                      <nav className="flex flex-col gap-1 lg:gap-2 w-full px-2">
                        <button
                          ...
                          className={cn(
                            ...
                            activeTab === "home"
                              ? "nav-pill-active"
                              : "hover:bg-[#EFEFED]/50 dark:hover:bg-[#2E2E2E]/50 text-[#757681] dark:text-[#9B9A97]",
                          )}
                        >
                          ...
                          <>
                            ...
                            {activeBusiness?.applets?.map((applet: any) => (
                              <button
                                key={applet.id}
                                onClick={() =>
                                  setActiveTab(`applet_${applet.id}` as any)
                                }
                                title={applet.name}
                                className={cn(
                                  "w-full flex items-center justify-center p-2.5 rounded-[12px] transition-colors relative group",
                                  activeTab === `applet_${applet.id}`
                                    ? "bg-[#EFEFED] dark:bg-[#2E2E2E] text-indigo-600 dark:text-indigo-400"
                                    : "hover:bg-[#EFEFED]/50 dark:hover:bg-[#2E2E2E]/50 text-[#757681] dark:text-[#9B9A97]",
                                )}
                              >
                  ...
                  <div
                    className={cn(
                      "flex-1 flex flex-col min-w-0 relative print:h-auto print:overflow-visible",
                      activeTab === "chat" && "md:flex h-full",
                  ...
                  >
                    ...
                    >
                      <div
                        className={cn(
                          "w-full flex-1 flex flex-col print:max-w-none print:h-auto print:block",
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <h1 className="text-xs md:text-base font-bold text-[#37352F] dark:text-[#EBE9ED]">
                                {activeTab === "home" && "Home"}
                                {activeTab === "schedule" &&
                                  industryConfig.terminology.calendar}
                                {activeTab === "search" &&
                                  industryConfig.terminology.products}
                                {activeTab === "brandkit" &&
                                  industryConfig.terminology.assets}
                          ...
                          >
                            <Calendar
                              ...
                              posts={
                                isAdmin
                                  ? posts
                                  : posts.filter((p) => !p.isHiddenForOthers)
                              }
                              ...
                              onRegeneratePost={
                                isAdmin ? handleRegeneratePost : undefined
                              }
                              onGenerateMockup={
                                isAdmin ? handleGenerateMockup : undefined
                              }
                              onUpdatePost={
                                isAdmin ? handleSavePost : undefined
                              }
                        ...
                        {isAdmin && (
                          <LazyTab
                            active={activeTab === "workspace_management"}
                          >
                            {() => (
                        ...
                        {isAdmin && (
                          <LazyTab
                            ...
                            className={cn(
                              "flex-1 pb-32 md:pb-12",
                              activeTab === "more" ? "block" : "hidden",
                            )}
                          >
                            {() => (
                              <SettingsView
                                  ...
                                  <button
                                    onClick={async () => {
                                      if (
                                        ...
                                        if (activeTab === `applet_${applet.id}`)
                                          setActiveTab("home");
                                      }
                                    }}
                                ...
                                <iframe
                                  ...
                                  srcDoc={
                                    applet.code.includes("<head>")
                                      ? applet.code.replace(
                                          "<head>",
                                          `<head><meta charset="UTF-8"><script src="https://cdn.tailwindcss.com"></script><script>window.FORGE_CONTEXT = ${JSON.stringify(activeBusiness || {})};</script>`,
                                        )
                      ...
                      <div className="opacity-80 scale-105  pointer-events-none">
                        {activeDragItem.type === "product" && (
                          <div className="bg-white dark:bg-[#191919] border border-brand rounded-[6px] p-3 w-64 ">
                            <div className="text-[10px] font-bold text-brand uppercase mb-1">
                              {activeDragItem.product.type}
                            </div>
                            <div className="text-sm font-bold">
                              {activeDragItem.product.title}
                            </div>
                          </div>
                        )}
              ...
              >
                {isAdmin ? (
                  <nav
                    className="flex-1 flex flex-row justify-between w-full h-[72px] items-stretch"
                  ...
                  <div className="flex-1 flex flex-row justify-between w-full h-[72px] items-center px-4">
                    <button
                      onClick={() => setActiveTab("schedule")}
                      aria-current={
                        activeTab === "schedule" ? "page" : undefined
                      }
                      className={cn(
              ...
              {dropActionPrompt && (
                <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
                  <div className="bg-white dark:bg-[#191919] rounded-[12px]  max-w-md w-full p-6 border border-[#E9E9E7] dark:border-[#2E2E2E]">
                    <h2 className="text-xl font-bold text-[#37352F] dark:text-[#EBE9ED] mb-4">
                      Multiple Images Dropped
                    </h2>
                    <p className="text-[#757681] dark:text-[#9B9A97] mb-6">
                      You dropped {dropActionPrompt.files.length} images. How
                    ...
                    <div className="flex flex-col gap-3">
                      <button
                        onClick={() =>
                          processDroppedFiles(
                            dropActionPrompt.dateStr,
                            dropActionPrompt.files,
                            "single",
                          )
                        }
                    ...
                    <div className="p-6 space-y-6">
                      <div className="space-y-4">
                        <div className="flex items-start gap-3">
                          <div className="w-6 h-6 bg-[#F7F7F5] dark:bg-[#2E2E2E] rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                            1
                          </div>
                          <p className="text-sm text-[#37352F] dark:text-[#EBE9ED]">
                            Open this site in your mobile browser (Safari for
                            iOS, Chrome for Android).
                          </p>
                        </div>

                        <div className="p-4 bg-[#F7F7F5] dark:bg-[#202020] rounded-[12px] border border-[#E9E9E7] dark:border-[#2E2E2E]">
                          <h3 className="text-xs font-bold text-[#9B9A97] dark:text-[#7D7C78] uppercase tracking-widest mb-3">
                            On iPhone (Safari)
                          </h3>
                          <ol className="text-sm space-y-2 list-decimal pl-4">
                            <li>
                              Tap the <span className="font-bold">Share</span>{" "}
                              button (square with arrow up) at the bottom.
                            </li>
                            <li>
                              Scroll down and tap{" "}
                            ...
                            <li>
                              Tap <span className="font-bold">Add</span> in the
                              top right corner.
                            </li>
                          </ol>
                        </div>
                      ...
                      <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-[12px] border border-blue-100 dark:border-blue-900/30">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-300">
                          This creates a shortcut on your home screen for
                          instant access. It doesn't use extra storage like a
                          regular app.
                        </p>
                      </div>
                    </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
                  <div className="bg-white dark:bg-[#191919] rounded-[12px]  border border-[#E9E9E7] dark:border-[#2E2E2E] w-full max-w-md p-6">
                    ...
                    <p className="text-[#757681] dark:text-[#9B9A97] mb-6">
                      ...
                      {confirmAction.type.toLowerCase()}? This action cannot be
                      undone.
                    </p>
                  ...
                  >
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm text-[#757681] dark:text-[#9B9A97]">
                        This calendar is password protected. Please enter the
                        password to view.
                      </p>
                    </div>
                    <div className="space-y-4">
