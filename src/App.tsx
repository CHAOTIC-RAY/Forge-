...
const PostModal = React.lazy(() =>
  import('./components/modals/PostModal').then((m) => ({ default: m.PostModal })),
);
const ImageViewer = React.lazy(() =>
  import('./components/ImageViewer').then((m) => ({ default: m.ImageViewer })),
);
const CalendarSharing = React.lazy(() =>
  import('./components/CalendarSharing').then((m) => ({ default: m.CalendarSharing })),
);
const DirectSearch = React.lazy(() =>
  import('./components/DirectSearch').then((m) => ({ default: m.DirectSearch })),
);
...
const ExcelImportModal = React.lazy(() =>
  import('./components/modals/ExcelImportModal').then((m) => ({ default: m.ExcelImportModal })),
);
const BusinessModal = React.lazy(() =>
  import('./components/modals/BusinessModal').then((m) => ({ default: m.BusinessModal })),
);
const AutoFillModal = React.lazy(() =>
  import('./components/modals/AutoFillModal').then((m) => ({ default: m.AutoFillModal })),
);
const WidgetsTab = React.lazy(() =>
  import('./components/CreativeStudioTab').then((m) => ({ default: m.WidgetsTab })),
);
...
function DroppableZone({
  ...
  return (
    ...
    >
      <div className={cn('p-3 rounded-full', isOver ? 'bg-white/20' : 'bg-white/10')}>{icon}</div>
...
export default function App() {
  ...
  const isViewer = useMemo(
    () => !!(user && activeBusiness && activeBusiness.memberRoles?.[user.uid] === 'viewer'),
    [user, activeBusiness],
  );
  const isGuest = !user;
  ...
  useEffect(() => {
    const handleExtensionMessage = async (event: MessageEvent) => {
      ...
      if (
        (event.data?.type === 'FORGE_ADD_NOTE' || event.data?.type === 'FORGE_QUICK_NOTE') &&
        user
      ) {
        try {
          ...
          toast.success(
            event.data.type === 'FORGE_ADD_NOTE' ? 'Note clipped to Ideas!' : 'Quick idea added!',
          );
        } catch (error) {
  ...
  const addSyncLog = (message: string, type: 'info' | 'success' | 'error' = 'info') => {
    setSyncLogs((prev) =>
      [{ id: uuidv4(), time: new Date(), message, type }, ...prev].slice(0, 100),
    );
  };
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (!isViewOnly && !loadingBusinesses) {
          const shouldOnboard = bizList.length === 0 && userOnboardingComplete !== true;
  ...
  const handleSavePost = async (post: Post) => {
    ...
    try {
      ...
      const postWithUser = {
        ...
        userId: user.uid,
        businessId:
          post.businessId || (calendarMode === 'personal' ? 'personal' : activeBusiness?.id || ''),
        images: imageUrls,
      };
    ...
    const checkScheduledPosts = async () => {
      ...
      try {
        ...
        const due = posts.filter(
          (p) =>
            p.publishStatus === 'scheduled' && p.scheduledTime && new Date(p.scheduledTime) <= now,
        );
        ...
        const repeatPosts = posts.filter(
          (p) =>
            p.repeatEnabled && p.publishStatus === 'published' && p.repeatInterval && p.publishedAt,
        );
        ...
        if (repeatPosts.length > 0) {
          ...
          for (const post of repeatPosts) {
            ...
            const intervalDays =
              post.repeatInterval === 'weekly' ? 7 : post.repeatInterval === 'biweekly' ? 14 : 30;
  ...
  const getDefaultPostOutlet = () => activeBusiness?.name?.trim() || OUTLETS[0] || 'Main Store';
    ...
    if (mode === 'single') {
      ...
      for (const file of files) {
        ...
        try {
          ...
          console.error('Failed to generate post from image:', error);
          const errMsg =
            error instanceof Error ? error.message : 'Failed to auto-generate content.';
          toast.error(errMsg);
          await handleSavePost({
            ...placeholderPost,
            title: 'New Post',
  ...
  const exportToExcel = async (settings: ExportSettings) => {
    ...
    const fetchImageAsBase64 = async (
      url: string,
    ): Promise<{ base64: string; extension: string; width: number; height: number } | null> => {
      ...
      <div className="min-h-screen bg-[#1A1C1E] flex items-center justify-center p-4">
        <div className="bg-[#24262B] border border-[#33353B] p-8 rounded-[24px] text-center max-w-md ">
          ...
          <p className="text-[#909094] mb-6">
            We're having trouble connecting to the cloud. This might be due to a slow network or a
  ...
  const handleExportExtensionZip = async () => {
    try {
      ...
      zip.file(
        'data/products.json',
        JSON.stringify(
          localStorage.getItem(`rainbowStockCheck_${activeBusiness?.id || 'default'}`)
            ? JSON.parse(
                localStorage.getItem(`rainbowStockCheck_${activeBusiness?.id || 'default'}`)!,
              )
            : [],
  ...
  const handleOnboardingComplete = async (
    ...
    try {
      ...
      ensureLocalAiEnginesReady()
        ...
        .catch(() =>
          toast.success('Workspace created — open Settings to finish loading local AI.'),
        );
  ...
  return (
    <ErrorBoundary>
      <SkipLink />
      {showOnboarding && user && (
        <OnboardingWizard userEmail={user.email || ''} onComplete={handleOnboardingComplete} />
      )}
                ...
                items={[
                  {
                    ...
                    disabled:
                      isIdeasTabActive || activeTab === 'brandkit' || activeTab === 'analytics',
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          (activeTab === 'chat' || activeTab === 'home') && 'max-w-none h-full',
                        )}
                      >
                        {/* Page Title */}
                        <div
                        ...
                        >
                          ...
                          >
                            <Calendar
                              ...
                              onGenerateWithAi={
                                isAdmin ? () => setIsAutoFillModalOpen(true) : undefined
                              }
                        ...
                        {isAdmin && (
                          <div
                            className={cn('flex-1', activeTab === 'search' ? 'block' : 'hidden')}
                          >
                            <LocalDb
                              onAddPost={(products) => {
                                setIsPostModalOpen(true);
                          ...
                          activeBusiness?.applets?.map((applet) => (
                            <div
                              ...
                              className={cn(
                                ...
                                activeTab === `applet_${applet.id}` ? 'block' : 'hidden',
                              )}
                            >
                                    ...
                                    onClick={async () => {
                                      if (confirm(`Delete the applet ${applet.name}?`)) {
                                        await updateDoc(doc(db, 'businesses', activeBusiness.id), {
                                          applets: activeBusiness.applets?.filter(
                                            (a) => a.id !== applet.id,
                                          ),
                                        });
                ...
                <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
                  <div className="bg-white dark:bg-[#191919] rounded-[12px]  max-w-md w-full p-6 border border-[#E9E9E7] dark:border-[#2E2E2E]">
                    ...
                    <p className="text-[#757681] dark:text-[#9B9A97] mb-6">
                      You dropped {dropActionPrompt.files.length} images. How would you like to add
                      them?
                    </p>
                    <div className="flex flex-col gap-3">
                      <button
                    ...
                    <div className="p-6 space-y-6">
                      <div className="space-y-4">
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-sm text-[#37352F] dark:text-[#EBE9ED]">
                            Open this site in your mobile browser (Safari for iOS, Chrome for
                            Android).
                          </p>
                        </div>
                        ...
                        <div className="p-4 bg-[#F7F7F5] dark:bg-[#202020] rounded-[12px] border border-[#E9E9E7] dark:border-[#2E2E2E]">
                          ...
                          <ol className="text-sm space-y-2 list-decimal pl-4">
                            <li>
                              Tap the <span className="font-bold">Menu</span> (three dots) in the
                              top right.
                            </li>
                            <li>
                      ...
                      <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-[12px] border border-blue-100 dark:border-blue-900/30">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-300">
                          This creates a shortcut on your home screen for instant access. It doesn't
                          use extra storage like a regular app.
                        </p>
                      </div>
                    </div>
              ...
              {confirmAction && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
                  <div className="bg-white dark:bg-[#191919] rounded-[12px]  border border-[#E9E9E7] dark:border-[#2E2E2E] w-full max-w-md p-6">
                    ...
                    <p className="text-[#757681] dark:text-[#9B9A97] mb-6">
                      Are you sure you want to {confirmAction.type.toLowerCase()}? This action
                      cannot be undone.
                    </p>
                  ...
                  >
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm text-[#757681] dark:text-[#9B9A97]">
                        This calendar is password protected. Please enter the password to view.
                      </p>
                    </div>
                    <div className="space-y-4">
                      <input
