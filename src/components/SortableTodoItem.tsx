...
export function SortableTodoItem({ todo, toggleTodo, deleteTodo, activeTab, group, priorityColors, businesses }: any) {
  ...
  return (
    <div 
      ...
      {...listeners}
      className={cn(
        "group flex flex-col gap-3 p-4 bg-white dark:bg-[#191919] border border-[#E9E9E7] dark:border-[#2E2E2E] rounded-xl transition-all hover:border-[#2665fd] cursor-grab active:cursor-grabbing",
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <button 
          ...
          className="opacity-0 group-hover:opacity-100 p-1.5 text-[#808080] hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
        >
      ...
      <div className="flex items-center gap-3 mt-auto pt-2 border-t border-[#F0F0F0] dark:border-[#2E2E2E]">
        {todo.dueDate && (
          <span className={cn(
            "flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md bg-[#F7F7F5] dark:bg-[#202020]",
        ...
        {activeTab !== group && todo.project && (
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#808080] flex items-center gap-1 px-2 py-1 rounded-md bg-[#F7F7F5] dark:bg-[#202020]">
